...
| **Phase 3** 🔄진행중 | 나머지 코드 순차 이식. **A-1·A-2·A-3 + C-1·C-2·C-3·C-4 구현 완료 (8/18)**. A-1/A-2는 keyword_info 1콜 저비용(LLM 없음), A-3는 실시간 CEP 7W 분류(`classifyCep`). C-1/C-2는 공용 브랜드 지형 파이프라인(`brands-shared.ts` — LLM 브랜드 추출 + 부분문자열 재검증으로 환각 방지, SoV는 "검색량 기준 근사" 강제)·C-2는 자사 브랜드 필수 입력, C-3는 path_finder 커넥터 신규(베타 — nodes/edges 방어 파싱, **실호출 미검증**), C-4는 실시간 페인포인트 분류(`classifyPainpoints`, 중립 키워드 억지 분류 금지). 폼은 코드별 입력 구성(`CODE_FORM`: 브랜드 필수/선택, 라벨 오버라이드)까지 일반화. 남은 것: D-1·D-3·D-4, A-4·A-5, P-1a/1b/2b, D-2·P-2a(미지원 유지) | A·C 밴드 완료, D/P 밴드 대기 |
