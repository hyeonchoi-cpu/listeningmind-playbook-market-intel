...
| **Phase 3** 🔄진행중 | 나머지 코드 순차 이식. **A-1~3 + C-1~4 + D-1·D-3·D-4 구현 완료 (11/18)**. A-1/A-2는 keyword_info 1콜 저비용(LLM 없음), A-3는 실시간 CEP 7W 분류(`classifyCep`). C-1/C-2는 공용 브랜드 지형 파이프라인(`brands-shared.ts` — LLM 브랜드 추출 + 부분문자열 재검증으로 환각 방지, SoV는 "검색량 기준 근사" 강제)·C-2는 자사 브랜드 필수 입력, C-3는 path_finder 커넥터 신규(베타 — nodes/edges 방어 파싱, **실호출 미검증**), C-4는 실시간 페인포인트 분류(`classifyPainpoints`, 중립 키워드 억지 분류 금지). 폼은 코드별 입력 구성(`CODE_FORM`: 브랜드 필수/선택, 라벨 오버라이드)까지 일반화. D-1은 CEP 7W + 논브랜드 비중으로 선점 점수(가정 명시), D-3는 자사 강점/경쟁 커버 갭/논브랜드 기회 3축(자사 브랜드 필수), D-4는 rels 인접 그래프 기반 라이징 연관 아이템 + 함께 검색 상황 해석. 남은 것: A-4·A-5, P-1a/1b/2b, D-2·P-2a(미지원 유지) | A·C·D 밴드 완료, P 밴드 대기 |
